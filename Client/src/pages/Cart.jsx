...
const Cart = () => {
   ...
   const getCart = () => {
    ...
    for (const key in cartItems) {
        ...
        if (product) {
            const safeOfferPrice = Number(product.offerPrice) || 0
            tempArray.push({
                ...product,
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                ...
                <button onClick={()=>{navigate("/products"); scrollTo(0,0)}} className="group cursor-pointer flex items-center mt-8 gap-2 text-indigo-500 font-medium">
                    ...
                    <p className="text-primary">Continue Shopping</p>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                        ...
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                                {adresses.map(( adresses, index)=>(
                                    ...
                                    <p onClick={() => {setSelectedAdress(adresses); setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
                                    {adresses.street},{adresses.city},{adresses.state},{adresses.country}
                                </p>

                            )) }
                                <p onClick={() => navigate("/add-adress")} className="text-primary text-center cursor-pointer p-2 hover:bg-indigo-500/10">
                                    Add address
                                </p>
                            </div>
                        )}
                ...
                <div className="text-gray-500 mt-4 space-y-2">
                    ...
                    <p className="flex justify-between text-lg font-medium mt-3">
                        <span>Total Amount:</span>{currency}{getCartAmount() + getCartAmount() * 2 / 100}<span>
                            </span>
                    </p>
                </div>
